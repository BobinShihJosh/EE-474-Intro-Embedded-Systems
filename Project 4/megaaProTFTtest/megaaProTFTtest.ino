...
void setup(void) {
  ...
  setUp();
  buttons[0].initButton(&tft, 70, 160, 80, 20, ILI9341_GREEN, ILI9341_LIGHTGREY, ILI9341_GREEN,
                          "menu", 1); 
  buttons[0].drawButton();
    
  buttons[1].initButton(&tft, 70, 190, 80, 20, ILI9341_GREEN, ILI9341_LIGHTGREY, ILI9341_GREEN,
                          "annunciation", 1); 
  buttons[1].drawButton();
  
  buttons[2].initButton(&tft, 70, 130, 80, 20, ILI9341_GREEN, ILI9341_LIGHTGREY, ILI9341_GREEN,
                         "-", 1); 
  buttons[2].drawButton();
    
  buttons[3].initButton(&tft, 70, 220, 80, 20, ILI9341_GREEN, ILI9341_LIGHTGREY, ILI9341_GREEN,
                          "-", 1); 
  ...
  void TFTkeypadSubsystemFunction(void *data)
{
  ...
  void MeasureSubsystemFunction(void *data) {
    ...
    if (*(measureData->functionSelectPtr) == 1) {
      Serial1.print('A');
      Serial.println(F("flag sent"));
    } else if (*(measureData->functionSelectPtr) == 2) {
      Serial1.print('B');
    }
    //delay(100);
    recvWithEndMarker();
...
//======================================Scheduler task============================================
  ...
  void schedulerSubsystemFunction(void* data) {
    ...
    //  insert(&statusTask);
      ...
      if (*(scheduleData->functionSelectPtr) != 0) {
        insert(&measureTask);
        temp = temp->next;
