...
void loop() {
  ...
  if (Serial.available()>0)
    intByte = Serial.read();
  if (intByte == '0') //'0' is the flag to run Temperature Measurement in Uno
    Serial.println(temperature);
  if (intByte == '1') // '1' is the flag to run Blood Pressure Measurement in Uno
    { 
      ...
      {
     // Serial.print("\n\t");Serial.print(pressure_buff);Serial.print("\t")
      // Serial.print("Pressure sys is:");Serial.println(pressure_sys);
      // Serial.print("Pressure dia is:");Serial.println(pressure_dia);
      }
      
    }
  if (intByte == '2') // '2' is the flag to run the Pulse Rate Measurement in Uno
    Serial.println(2*pulserate); //500ms
  if (intByte == '3') // '3' is the flag to run the Respiration Rate Measurement in Uno
